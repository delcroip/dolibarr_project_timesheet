...
require_once '../modules_timesheet.php';
...
class pdf_rat extends ModelPDFTimesheetReport
{
	 ...
	 *	@param	date        $startday	first day of the report
	 *	@param	date        $stopday	lastday of the report
	 *	@return	int         				1 if OK, <=0 if KO
	 */
	function write_file($object,$outputlangs,$startday='',$stopday='')
	{
		...
		if ($conf->projet->dir_output)
		{
			...
			//$dir = $conf->projet->dir_output;
                    $dir=DOLIBARR_MAIN_DOCUMENT_ROOT.'./';
			...
			if (file_exists($dir))
			{
				...
				// Complete object by loading several other informations
                                $report=new TimesheetReport($this->db);
                                $report->initBasic($object->id, '', '');
				//$task = new Task($this->db);
				//$tasktimearray = $task->getTasksArray(0,0,$object->id);
                                $tasktimearray=$report->getReportArray($startday,$stopday,' ORDER BY task_date,taskid,userid');
                                $TotalLines=0;
                                foreach($tasktimearray as $line){$TotalLines+=$line['duration'];}
                                $tasktimearray[]=array('projectId'=>'','projectLabel'=>convertSecondToTime($TotalLines,'allhourmin'),
                                    'taskId'=>'','taskLabel'=>convertSecondToTime($TotalLines,'allhourmin'),  
                                    'userId'=>'','userName'=>$outputlangs->transnoentities('Total') .cdlt,'duration'=>0,
                                    'date'=>'');
				...
				$pdf->SetKeyWords($outputlangs->convToOutputCharset($object->ref)." ".$outputlangs->transnoentities("Project")." ".$outputlangs->transnoentities("Timesheet"));
				...
				if ($showSign=1)
				{
					...
					$pdf->SetFont('','', $default_font_size - 1);
					$pdf->writeHTMLCell(80, 3, $this->marge_gauche+1, $bottomlasttab+1, $outputlangs->convToOutputCharset('SupplierSignature'), 0, 1);
                                        $pdf->writeHTMLCell(80, 3, $this->marge_gauche+$widthSignBox+2, $bottomlasttab+1, $outputlangs->convToOutputCharset('CustomerSignature'), 0, 1);
					$nexY = $pdf->GetY();
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("ProjectTimeReport")." ".$outputlangs->convToOutputCharset($object->ref), '', 'R');
		$pdf->SetFont('','', $default_font_size + 2);

		$posy+=6;
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("DateStart")." : " . dol_print_date($startday,'day',false,$outputlangs,true), '', 'R');

		$posy+=6;
		$pdf->SetXY($posx,$posy);
		$pdf->MultiCell(100, 4, $outputlangs->transnoentities("DateEnd")." : " . dol_print_date($stopday,'day',false,$outputlangs,true), '', 'R');
