...
// show the field timsesheetuser
		...
		if($edit==1){
                    //avoid full table scan print select_generic('timesheet_user', 'rowid','Timesheetuser','CONCAT(u.lastname," ",u.firstname) AS username','CONCAT(t.start_date," ",t.target) AS weektarget',$object->timesheetuser,' - ',$sqltail);
                    print '<input class="flat" size="5" type="text" name="Timesheetuser" value="'.$object->timesheetuser.'"/>';

		}else{
                    print print_generic('timesheet_user', 'rowid',$object->timesheetuser,'CONCAT(lastname," ",firstname) AS username','CONCAT(start_date," ",target) AS weektarget',' - ','',$sqltail);
                    //print $object->project_tasktime_list;
