...
class AttendanceSystemUserLink extends CommonObject
{
	...
	global $conf, $langs;
        ...
        if($id){
            $lien = '<a href = "'.dol_buildpath('/timesheet/AttendanceSystemUserLinkCard.php',1).'?id='.$id.'&action=view"'.$linkclose.'>';
        }else if (!empty($ref)){
            $lien = '<a href = "'.dol_buildpath('/timesheet/AttendanceSystemUserLinkCard.php',1).'?ref='.$ref.'&action=view"'.$linkclose.'>';
        }else{
            $lien = "";
        }
...
// Commit or rollback
    ...
    */
    ...
    Public function sellist($selected = ''){    
        ...
        $html = array('name' => 'AttendanceSystemUserLink', 'class' => '', 'otherparam' => '', 'ajaxNbChar' => '', 'separator' => '-');
        $addChoices = null;
