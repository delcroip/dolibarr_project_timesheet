...
public function getHTMLFooter($ajax = false)
{
    ...
    if($isOpenSatus) {
        $html .= '<input type = "submit"  onKeyDown="removeUnchanged();" class = "butAction" name = "save" value = "'.$langs->trans('Save')."\" />\n";
        ...
        if(in_array('1', array_slice($apflows, 1))) {
            $html .= '<input type = "submit" onKeyDown="removeUnchanged();" class = "butAction" name = "submit"  value = "'.$langs->trans('Submit')."\" />\n";
        }
