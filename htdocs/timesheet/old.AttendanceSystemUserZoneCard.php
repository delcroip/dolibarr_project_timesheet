...
//if(isset( $_SESSION['AttendanceSystemUserclass'][$tms]))
//{
...
$error=0;
if ($cancel){
        AttendanceSystemUserZoneReloadPage($backtopage,$id,$ref);
}else if (($action == 'add') || ($action == 'update' && ($id>0 || !empty($ref))))
{
    //block resubmit
    if(empty($tms) || (!isset($_SESSION['AttendanceSystemUserZone'][$tms]))){
 ...
 }else if ($id==0 && $ref=='' && $action!='create') 
 {
  ...
  switch($action){		
    case 'update':
        ...
        {
            // Creation OK
            unset($_SESSION['AttendanceSystemUserZone'][$tms]);
    ...
    case 'add':
        ...
        if ($result > 0)
        {
            ...
            // remove the tms
               ...
               AttendanceSystemUserZoneReloadPage($backtopage,$result,'');
     ...
     case 'confirm_delete':
            ...
            AttendanceSystemUserZoneReloadPage($backtopage, 0, '');
         break;
...
if(isset( $_SESSION['AttendanceSystemUserZone'][$tms]))
{
    unset($_SESSION['AttendanceSystemUserZone'][$tms]);
}
if(($action == 'create') || ($action == 'edit' && ($id>0 || !empty($ref)))){
    $tms=getToken();
    $_SESSION['AttendanceSystemUserZone'][$tms]=array();
    $_SESSION['AttendanceSystemUserZone'][$tms]['action']=$action;
...
switch ($action) {
   ...
   case 'delete';
        if( $action=='delete' && ($id>0 || $ref!="")){
         $ret=$form->form_confirm($PHP_SELF.'?action=confirm_delete&id='.$id,$langs->trans('DeleteAttendanceSystemUserZone'),$langs->trans('ConfirmDelete'),'confirm_delete', '', 0, 1);
	...
	print '<div class="center">';
        ...
        if($new==1){
            ...
            if (empty($reshook))
            {
                ...
                // Boutons d'actions
                //if($user->rights->AttendanceSystemUserZone->edit)
                //{
                    print '<a href="'.$PHP_SELF.'?id='.$id.'&action=edit" class="butAction">'.$langs->trans('Update').'</a>';
                //}
                
                //if ($user->rights->AttendanceSystemUserZone->delete)
                //{
