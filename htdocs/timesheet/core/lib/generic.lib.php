...
//getConf('COMPANY_USE_SEARCH_TO_SELECT')
//getConf('PRODUIT_USE_SEARCH_TO_SELECT')
//getConf('PROJECT_USE_SEARCH_TO_SELECT')
//getConf('RESOURCE_USE_SEARCH_TO_SELECT')
//getConf('BARCODE_USE_SEARCH_TO_SELECT')
//getConf('CONTACT_USE_SEARCH_TO_SELECT')
...
function getConf($cName, $default = false){
    global $conf;
    if (property_exists($conf->global, $cName)){
        return $conf->global->$cName;
    }
    else {
        return $default;
    }
    ...
    $sqlarray = array('table' => 'user', 'keyfield' => 'rowid', 'fields' => 'firstname, lastname', 'join' => '', 'where' => '', 'tail' => ''), $htmlarray = array('name' => 'HTMLSellist', 'class' => '', 'otherparam' => '', '$ajaxNbChar' => '', 'separator' => ' ', 'noajax' => 0), $selected = '', $addtionnalChoices = array('NULL' => 'NULL'))
{
    ...
    if (!isset($htmlarray['otherparam'])) {
        $htmlarray['otherparam'] = '';
    }
    if ($ajaxNbChar && $ajax) {
        if ($selectedValue == '' && is_array($addtionnalChoices) && isset($addtionnalChoices[$selected])) {
...
function print_sellist(
    $sqlarray = array('table' => 'user', 'keyfield' => 'rowid', 'fields' => 'firstname, lastname', 'join' => '', 'where' => '', 'tail' => ''), $selected = '', $separator = ' ', $url = '')
{
    global $conf, $langs, $db;
    $select = "";
    ...
    if ($resql) {
        ...
        if ($num) {
            ...
            if ($obj) {
                ...
                if (!empty($url))
                    $select = '<a href = "' . $url . $obj->{$sqlarray['keyfield']} . '">' . $select . '</a>';
            } else {
                $select = "NULL";
            }
        } else {
...
function print_generic($table, $fieldValue, $selected, $fieldToShow1, $fieldToShow2 = "", $separator = ' - ', $sqltail = "", $sqljoin = "")
{
    ...
    return print_sellist(
        $sqlarray = array('table' => $table, 'keyfield' => $fieldValue, 'fields' => $fieldToShow1 . (empty($fieldToShow2) ? '' : ', ' . $fieldToShow2), 'join' => $sqljoin, 'where' => '', 'tail' => $sqltail),
        $selected,
        $separator
    );
}
